...
public class GuessingGameSecondEnhancement {
    private static int RandomNumber( int upper ) {
        int random;
        int lower = 1;
        random = lower + ( int )( Math.random() * ( upper - lower + 1 ) );
        return random;
    }
                ...
                case 2:
                    ...
                    for ( int i = 1; i <= gameNum; i++ ) {
                        ...
                        for ( int j = 0; j < games.size(); j++ ) {
                            j = k;
                            k++;
                            if ( games.get( j ) == 15000 ) {
                                break;
                            }
                            printAll += games.get( j ) + ", ";
                        }
                        out.println( "For Game " + i + " you put in numbers " + printAll.substring( 0, printAll.length() - 2 ) + "." );
                    }
                    out.println( "GoodBye!" );
                    break;
            }
